# class for managing the LDAP integration
...
class LDAPBackend(object):
	...
	def _create_user_object(self, username, password):
		  ...
		  user = User(username=username, password=password, is_staff=True)		
		  return user 
	...
	def _update_user(self, l, user):
		...
		if ('LDAP_ACTIVE_FIELD' in self.settings
			...
			user.is_active = True
		user.save()
		# added MIKELE for managing editors
		try: 
			# the 'default_editor' group should be created beforehand 
			g = Group.objects.get(name="default_editor")
		except Group.DoesNotExist: 
			# creates a new group: permissions need to be added manually!
			g = Group(name="default_editor")
			# NEW probably cause of crash...
			g.save()
		user.groups.add(g)
